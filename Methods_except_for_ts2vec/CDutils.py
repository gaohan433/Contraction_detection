...
def rms_cd(rms_envelope,window,threhold_factor):
    half_window = window//2
    thr = np.mean(rms_envelope)+threhold_factor*np.std(rms_envelope)
    c = rms_envelope>thr
    c1 = np.zeros_like(c)
    for i in range(len(c)):
        if c[i]==1:
            l = max(0,i-half_window)
            r = min(i+half_window,len(c)-1)
            c1[l:r]=1
    return c1
...
# def cp_score1(gt_cpds,pre_cpds,time_series_l):
#     if len(pre_cpds)>len(gt_cpds):
#         sum_diff = 0
#         for i in range(len(gt_cpds):
#             value = find_closest_value(pre_cpds,gt_cpds[i])
#             sum_diff = sum_diff+np.abs(value-pre_cpds[i])
#         for i in range(len(pre_cpds)-len)
...
# Function to segment the time series using a sliding window and compute the energy of each segment
def segment_time_series(time_series, window_size, step_size):
    energies = []
    indices = []
    for start in range(0, len(time_series) - window_size + 1, step_size):
        end = start + window_size
        window = time_series[start:end]
        energy = compute_energy(window)
        energies.append(energy)
        indices.append((start, end))
    return np.array(energies), indices
